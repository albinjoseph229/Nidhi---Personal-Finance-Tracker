...
// ... (Interface Category, investmentCategories, getPlaceholderColor, and SellInvestmentModal remain exactly the same)
interface Category {
  ...
  return (
    ...
    >
      <Pressable style={styles.modalBackdrop} onPress={onClose}>
        <Pressable
          style={{ width: "80%", padding: 20, backgroundColor: cardColor, borderRadius: 20 }}
          onPress={() => {}}
        >
          <ThemedText style={{ fontSize: 18, fontWeight: "600", textAlign: "center" }}>
            Sell Investment
          </ThemedText>
          <ThemedText style={{ textAlign: "center", marginVertical: 8, opacity: 0.7 }}>
            Enter the final selling price per unit.
          </ThemedText>
          <TextInput
            style={[
              styles.textInput,
...
export default function AddInvestmentScreen() {
  ...
  const { investments, addInvestment, updateInvestment, deleteInvestment, addTransaction } =
    useAppData();
  const { theme } = useTheme();
  const isEditMode = !!params.uuid;

  const [name, setName] = useState("");
  const [selectedType, setSelectedType] = useState<Investment["type"] | null>(
    null
  );
  ...
  const placeholderColor = getPlaceholderColor(theme);
  const switchThumbColor = Platform.OS === "android" ? "#ffffff" : "";

  // ... (showDatePicker, handleDelete, and confirmSell functions remain the same)
  ...
  const confirmSell = async (priceString: string) => {
    ...
    if (isNaN(numSoldPrice) || numSoldPrice < 0) {
      return Alert.alert(
        "Invalid Price",
        "The entered price is not a valid number."
      );
    }
    ...
    try {
      ...
      await addTransaction({
        ...
        notes: `Sale of ${investmentToSell.name}`,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode) {
        ...
        // --- ✅ START: MODIFIED LOGIC WITH CONDITIONAL ---
        // Only create an expense transaction if the user wants to
        // track this purchase as a cash outflow.
        if (trackAsExpense) {
          const totalPurchaseCost = invData.quantity * invData.purchasePrice;
          await addTransaction({
            type: "expense",
            amount: totalPurchaseCost,
            category: "Investment",
            date: invData.purchaseDate,
            notes: `Purchase of ${invData.name}`,
          });
        }
        // --- ✅ END: MODIFIED LOGIC WITH CONDITIONAL ---
      }
      if (router.canGoBack()) router.back();
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        `Could not ${isEditMode ? "update" : "save"} the investment.`
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const isFormValid =
    name && selectedType && quantity && purchasePrice && currentValue;
  const isSold = status === "sold";

  return (
    <ThemedView style={styles.container}>
      {/* ... (StatusBar, SellInvestmentModal, KeyboardAvoidingView, Header are the same) */}
      ...
      >
          ...
          <View style={styles.headerActions}>
            {isSold && (
              <View style={styles.soldBadge}>
                <ThemedText style={styles.soldBadgeText}>SOLD</ThemedText>
              </View>
            )}
            {isEditMode && (
        ...
        >
          {/* ... (Asset Name and Type cards are the same) */}
          <ThemedView
            style={[
              styles.card,
            ...
            <View style={styles.categoryContainer}>
              {investmentCategories.map((item) => (
                <Pressable
                  ...
                  onPress={() => !isSold && setSelectedType(item.name)}
                >
                  <Feather
                    name={item.icon}
                    size={24}
                    color={selectedType === item.name ? "white" : textColor}
                  />
                  <ThemedText
                    style={[
                      styles.categoryText,
          ...
          >
            ...
            {/* ... (Input rows for Quantity, Purchase Price, Current Value are the same) */}
            ...
            {/* --- ✅ START: NEW UI FOR THE TOGGLE --- */}
            {/* This option is only shown for NEW investments */}
            {!isEditMode && (
              <View style={styles.switchContainer}>
                ...
                <Switch
                  ...
                  onValueChange={setTrackAsExpense}
                  value={trackAsExpense}
                />
              </View>
            )}
            {/* --- ✅ END: NEW UI FOR THE TOGGLE --- */}
            
            {/* ... (Sold Price and Purchase Date sections are the same) */}
            {isSold && soldPrice !== undefined && (
              <View style={[styles.inputContainer, { marginTop: 16 }]}>
                ...
                <TextInput
                  style={[styles.textInput, { color: "#4CAF50", fontWeight: "bold" }]}
                  value={String(soldPrice)}
                  editable={false}
                />
              </View>
            )}
            ...
            >
              ...
              <View style={styles.detailContent}>
                ...
                <ThemedText style={styles.detailValue}>
                  {purchaseDate
                    ? formatDateForDisplay(purchaseDate.toISOString())
                    : "Select Date"}
                </ThemedText>
              </View>
        ...
        {/* ... (Bottom container with buttons is the same) */}
          ...
          <Pressable
            style={[
              styles.saveButton,
              {
                ...
                marginTop: isEditMode && !isSold ? 12 : 0,
              },
              isSold && styles.saveButtonDisabled,
            ]}
            onPress={handleSave}
            disabled={!isFormValid || isSaving || isSold}
          >
            {isSaving ? (
              <ActivityIndicator color={saveButtonTextColor} />
            ) : (
              <ThemedText
                style={[styles.saveButtonText, { color: saveButtonTextColor }]}
              >
                {isEditMode ? "Update" : "Save Investment"}
              </ThemedText>
            )}
          </Pressable>
        </ThemedView>
...
const styles = StyleSheet.create({
  ...
  // --- ✅ START: NEW STYLE FOR THE SWITCH ---
  switchContainer: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
